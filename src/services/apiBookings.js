...
export async function getAllBookings({ filter, sortby, page }) {
  ...
  //filter
  if (filter) query = query[filter.filterMethod || 'eq'](filter.field, filter.value);

  //sort
  if (sortby) query = query.order(sortby.field, { ascending: sortby.direction === 'asc' })
  //ONly rows from this specific current page
  //pageSize=>total Number of pages
  if (page) {
  ...
  if (error) {
    ...
    throw new Error("Bookings data couldnot be loaded")
  }
  return {data,count}
}
...
export async function deleteBooking(id) {
  console.log(id)
  const { data, error } = await supabase.from('bookings').delete().eq('id', id).single();
  console.log('deleting from api booking')
  if (error) {
    console.log(error.message);
    throw new Error('Error occured while deleting Booking data. Please try again later!')
  }
  return data;
}
